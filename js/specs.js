function defineSpecsFor(apiRoot, servirtiumMode){
  ...
  function ajax(httpMethod, url, options) {
    ...
    if (servirtiumMode && url.startsWith("https://localhost:61417")) {
      expect.fail("All communication for the Servirtium compatibility test suite should go through Servirtium\n" +
    ...
    if (servirtiumMode === undefined && ! url.startsWith("http://localhost:61417")) {
      const domain = url.substring(0, url.indexOf("/", 10));
      expect.fail("All communication for the Servirtium compatability test suite should go through Servirtium\n" +
